...
#define RUNTIME_DATA_CONTENT ;
			...
			void SetAlphaBlendWriteMode(std::uint32_t a_alphaBlendWriteMode)
			{
				auto& data = GetRuntimeData();

				if (data.alphaBlendWriteMode != a_alphaBlendWriteMode) {
					data.alphaBlendWriteMode = a_alphaBlendWriteMode;
					data.stateUpdateFlags.set(ShaderFlags::DIRTY_ALPHA_BLEND);
				}
			}

			void SetDepthStencilDepthMode(DepthStencilDepthMode a_depthStencilDepthMode)
			{
				auto& data = GetRuntimeData();

				if (data.depthStencilDepthMode != a_depthStencilDepthMode) {
					data.depthStencilDepthMode = a_depthStencilDepthMode;
					if (data.depthStencilDepthModePrevious != a_depthStencilDepthMode) {
						data.stateUpdateFlags.set(ShaderFlags::DIRTY_DEPTH_MODE);
					} else {
						data.stateUpdateFlags.reset(ShaderFlags::DIRTY_DEPTH_MODE);
					}
				}
			}
		};
