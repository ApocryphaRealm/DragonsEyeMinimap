...
// Address Library guard (from Wheeler - Refined 1.2.9; here since Dragon's Eye Minimap 1.7.2). oproso on the Nexus page, 16-17 Sep 2026, running Fluorine on
...
namespace AddressLibraryGuard
{
	struct Report
	{
		...
		bool simulated = false;   // APOCRYPHA_SIMULATE_MISSING_ADDRESS_LIBRARY=1 forced the missing verdict
	};
	...
	inline Report Check()
	{
		...
		if (const char* sim = std::getenv("APOCRYPHA_SIMULATE_MISSING_ADDRESS_LIBRARY"); sim && sim[0] == '1') {
			r.existsAtCwd = false;
			r.simulated = true;
		}
